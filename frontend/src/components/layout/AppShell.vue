...
<template>
  <div class="flex h-full flex-col lg:flex-row">
    <SideNav v-if="props.showNav" />
    <main
      data-app-page
      :data-width="props.width"
      class="min-h-0 w-full flex-1 overflow-y-auto overscroll-contain"
    >
      <div
        data-testid="page-body"
        class="mx-auto w-full max-w-2xl px-4 pt-[max(1rem,env(safe-area-inset-top))] lg:px-8"
        :class="[
          props.showNav ? 'pb-10 lg:pb-8' : 'pb-[max(2rem,env(safe-area-inset-bottom))]',
          props.width === 'wide' ? 'lg:max-w-6xl' : 'lg:max-w-3xl',
        ]"
      >
