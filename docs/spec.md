...
- **Deployment:** Docker Compose on the homelab box, images built by GitHub Actions on a version tag and pushed to Docker Hub, behind whatever reverse proxy fronts that server
...
- `GroupMembers` - user to group, role
- `Expenses` - payer, amount, currency, description, date, receipt reference, group, recurrence rule (nullable), full edit history
- `ExpenseSplits` - expense to user, split type (equal / percentage / shares / exact amount / itemized), amount owed
...
- ~~Categories with icons + spending-by-category breakdown~~ dropped: it added a field to every expense form for a breakdown nobody read, and the icon now belongs to the group instead
...
- ~~**Auto-categorization**~~ dropped with categories
...
- Review wizard (same pattern as Settle Up import), extended with:
  ...
  - Editable split assignment per row
...
- Eight accent colours, chosen in the profile and kept on the account, which tint the whole app including its surfaces
- English and French, chosen in the profile and kept on the account; money and dates follow it
- One group at a time: the dashboard, activity and stats screens all follow it, and a sideways swipe moves between groups
...
| Deploy | Docker Compose, images built on a version tag and pushed to Docker Hub |
...
13. Deployment: Dockerfile(s), GitHub Actions on a version tag -> Docker Hub, compose on the server, backup cron

---

## Where this actually stands

Written after an audit of the code, so the list above reads as intent and this
reads as fact.

Built and in use: groups with one-at-a-time navigation, expenses with every split
type, settlements and simplified debts, the offline replica with vector clocks and
an outbox, invites by link and QR, the activity feed, the stats dashboard, both
importers (a Settle Up export and a bank statement, the statement parsed on the
device), conflicts, themes, languages, and the PWA with push.

On the server but not yet reachable from the app:

| Feature | Endpoint |
| --- | --- |
| Recurring expenses | `/api/expenses/recurring` (a worker creates them; nothing can add one) |
| Receipt photos | `/api/receipts` (the field is plumbed through the model and sync) |
| Merge two groups | `/api/groups/merge` |
| Move an expense between groups | `/api/expenses/{id}/transfer` |
| Nudge somebody to pay | `/api/settlements/nudge` |
| Cross-group settle view | `/api/settlements/overall` |
| Expense edit history | `/api/expenses/{id}/history` |
| Undo an import batch | `/api/import/batches/{id}/rollback` |
| Sign out everywhere | `/api/auth/signout-all` |

Known gaps, deliberately left:

- The activity feed's sentences are composed on the server in English, so they stay
  English in French. Translating them means sending the parts rather than the
  sentence.
- The icon picker's labels and search words are English only. They are search
  keywords as much as labels, so translating them means translating the search.
- Statement import understands the layouts it has been shown; a new bank's export
  may need its columns mapped by hand.
